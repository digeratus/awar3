...
## 2026-08-04 16:05 EDT — Production release completed

- Source: `f93dda3a1d00be8c37f7a907269b3e3fddeffe94` (`Release responsive daily mobile designs`) pushed to public GitHub repository `digeratus/awar3` on `main`.
- GitHub Actions: run `30945366914` completed `success` at `2026-08-04T19:54:51Z`; validation and Cloudflare deployment jobs both succeeded. GitHub deployment record `5750569630` reports `success` for the Cloudflare Pages environment.
- Cloudflare Pages: deployment UUID `15e4d481-571c-44eb-ba47-6f628c5b21cb`; deployment URL `https://15e4d481.awar3-astro-skunkworks.pages.dev`; production custom domain `https://awar3.com/` is serving the release.
- Active selection: New York date `2026-08-04`; `X-AWAR3-Variant: airborne-workshop`; `X-AWAR3-Candidate: b` (Fieldbook). The next New York date will select Living Systems / Candidate A (Board Stack). Desktop and iPhone user agents returned the same diagnostics.
- Production headers: normal root returns `Cache-Control: public, max-age=0, must-revalidate`, HSTS, self-hosted CSP, `X-Frame-Options: DENY`, and `X-Content-Type-Options: nosniff`. Valid design override returns `no-store` plus `X-Robots-Tag: noindex, nofollow`; invalid override uses the scheduled root. All candidate and responsive QA routes return `no-store` and noindex. `www` redirects to the canonical root.
- SEO: robots allows `/` and disallows `/variants/`; sitemap contains only `https://awar3.com/`.
- Browser/Chrome: live Browser 8/8 candidate routes at 390px, live Chrome 16/16 candidate cells at 390/768px, and Browser/Chrome root checks at 390/1440px all passed copy, image, structure, interaction, overflow, and console checks. Representative evidence is outside the repository under `../AWAR3_QA_Archive_2026-08-04/release-v1-production/`.
- Asset verification: all 24 production mobile AVIF/WebP files matched the local release SHA-256 byte-for-byte. The 12 MiB release gate and exact asset allowlist passed in CI.
- Rollback instructions: restore Cloudflare deployment `ad0eeb51` (`https://ad0eeb51.awar3-astro-skunkworks.pages.dev`) or revert `f93dda3` to prior source `8a0be233462bd83dc2b2169d7751b7832c259900`; rerun root, candidate-route, header, and Browser/Chrome smoke checks before declaring rollback complete. The prior desktop baseline source remains `f49a1427d41931094e4e4bfa5e586f992ef85396`.
